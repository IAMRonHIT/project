...
type ModelTranscriptCallback = (transcript: string) => void;
...
interface ConfigOptions {
  ...
  onModelTranscript?: ModelTranscriptCallback;
}
...
class RealtimeAudioService {
  ...
  public onModelTranscript: ModelTranscriptCallback | null = null;
  ...
  private setupDataChannelCallbacks(): void {
      ...
      try {
        ...
        if (message.type === 'transcript') {
          ...
          if (this.onTranscript) {
            this.onTranscript(message.text, message.is_final || false);
          }
        } else if (message.type === 'audio_transcript') {
          // Handle model transcript (what the model said)
          if (this.onModelTranscript) {
            this.onModelTranscript(message.text);
          }
        } else if (message.type === 'function_call') {
          // Handle function calls
          console.log('Function call received:', message.function_call);
          if (this.onFunctionCall) {
            this.onFunctionCall(message.function_call);
          }
        } else if (message.type === 'audio_started') {
          this.setConnectionState('speaking');
          console.log('Model speaking started');
        } else if (message.type === 'audio_finished') {
          this.setConnectionState('connected');
          console.log('Model speaking finished');
        }
        
        // Forward all messages to onMessage handler
        if (this.onMessage) {
    ...
    this.dataChannel.onclose = () => {
      ...
      // Only update state if it wasn't explicitly disconnected
